...
def gemini_prompt(question: str, context: str) -> str:
    prompt_in_chat = f"""Dựa vào thông tin trong ngữ cảnh, hãy đưa ra câu trả lời đầy đủ cho câu hỏi. Chỉ trả lời câu hỏi được hỏi, câu trả lời phải ngắn gọn và phù hợp với câu hỏi. Cung cấp số của tài liệu nguồn khi cần thiết. 
